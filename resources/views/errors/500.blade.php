...
<body>
    <main class="page fade-in">
        <div id="top-bar" class="top-bar">
            ...
            <span class="game-counter-panel"><span id="game-counter" class="game-counter">99</span> little bugs in the
                code</span>
        </div>
        <div class="flex-container" id="error-message">
            ...
            <div>
                <p>
                    Don't worry, we've been reported about that. In the meantime, you can help us <span
                        class="underlined">catch some nasty bugs</span>. We got you the deadliest of all weapons!
                </p>
            </div>
        </div>
        <div id="game-canvas">
            <svg id="slipper" height="250px" viewbox="0 0 120 273"
                style="position: absolute; bottom: 30px; right:30px; z-index:1;">
                <path id="slipper-shadow" style="fill:#000000;fill-opacity:0.4"
                ...
                <g id="slipper-body">
                    <path style="fill:#ffcc00;fill-opacity:1;"
                    ...
                    <path style="fill:#e50b0b;fill-opacity:1;"
...
<script>
    ...
    function init() {
        ...
        var slipper = document.getElementById("slipper"), // Target the whole slipper drawing (slipper + shadow)
            ...
            slipperOffsetX = slipper.getBoundingClientRect().width -
            slipperWidth, // slipper-body offsets relative to the whole svg
        ...
        function Bug(x1, y1, x2, y2) {
            ...
            this.deg = function() {
                return ((Math.atan((this.y2 - this.y1) / (this.x2 - this.x1))) + ((this.x2 - this.x1) / Math.abs(
                    this.x2 - this.x1)) * Math.PI / 2) * 180 / Math.PI;
            };
            this.t = function() {
        ...
        function mouseMoveHandler(e) {
            var mouseX = e.clientX - document.body.getBoundingClientRect()
                .left, // Calculates mouse coords in the context of game area - body element has a max-width set in CSS for big screens
                mouseY = e.clientY;

            var offset = document.getElementById("top-bar").getBoundingClientRect()
            .height; // Vertical offset to prevent the weapon to move over the topbar
            var newX, newY;
        ...
        function checkHit(e) {
            if (bugs) {
                ...
                bugs.forEach(function(b) {
                    ...
                    if ((bugX > mouseX - slipperWidth / 2) && (bugX < mouseX + slipperWidth / 2) && ((bugY >
                            mouseY - slipperHeight / 2) && (bugY < mouseY + slipperHeight / 2))) {
