$(document).ready(function () {
    ...
    $("#thumb_file").change(function (event) {
        ...
        if (fileInput) {
            ...
            if (
                ...
                const $imageElement = $(".preview-container img").attr(
                    "src",
                    objectURL
                );
                ...
                $("#thumb_error").text(
                    "Image extension must be jpg, jpeg, png, gif!"
                );
                $("#thumb_error").show();
            }
        }
    ...
    $("#submit-btn").click(function () {
        ...
        validateNull(
            thumb_image,
            "#thumb_error",
            "Please fill hotel room thumbnail image!"
        );
        validateNull(
            room_name,
            "#room_name_error",
            "Please fill hotel room name!"
        );
        validateNull(
            room_occupation,
            "#room_occupation_error",
            "Please fill hotel room occupation!"
        );
        validateNull(
            room_bed,
            "#room_bed_error",
            "Please choose hotel room bed type!"
        );
        validateNull(
            room_size,
            "#room_size_error",
            "Please fill hotel room size!"
        );
        validateNull(
            room_view,
            "#room_view_error",
            "Please choose hotel room view!"
        );
        validateNull(
            room_price,
            "#room_price_error",
            "Please fill hotel room price per day!"
        );
        validateNull(
            extra_bed_price,
            "#extra_bed_price_error",
            "Please fill hotel room extra bed price per day!"
        );
        validateNull(
            description,
            "#description_error",
            "Please fill hotel room description!"
        );
        validateNull(
            room_details,
            "#room_details_error",
            "Please fill hotel room details!"
        );

        validateCheckbox(
            room_amenity,
            "#room_amenity_error",
            "Please check hotel room amenity!"
        );
        validateCheckbox(
            room_feature,
            "#room_feature_error",
        ...
        if (room_name.length <= 2 && room_name != "") {
            $("#room_name_error").text(
                "Hotel room name must be greater then two!"
            );
